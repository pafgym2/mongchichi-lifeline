...
export async function selectHotIssues(candidates, maxN) {
  ...
  try {
    const res = await client.messages.create({
      ...
      max_tokens: 3000,
      system:
        "당신은 한국 종합일간지의 1면을 정하는 편집국장입니다. 후보 헤드라인 중 '오늘 한국에서 가장 중요한 뉴스'를 골라 한국어 1~2문장 요약을 작성합니다. 여러 매체가 동시에 다루거나 사회적 파급이 큰 사안을 우선하고, 연예·스포츠 가십이나 광고성·사소한 기사는 제외합니다.",
      messages: [
        {
          role: "user",
          content: `다음 후보 헤드라인 중 '오늘 가장 중요한 뉴스'를 최대 ${maxN}개 선별하세요.\n\n기준:\n- 정치·경제·사회·부동산 등에서 파급력이 크거나 여러 매체가 다루는 사안을 우선\n- 비슷한 주제(중복 보도)는 하나로 합치기\n- 사소하거나 광고성·가십 기사는 제외 (중요한 게 적으면 ${maxN}개보다 적어도 됨)\n- 중요도가 높은 순서로 정렬\n\n후보:\n${list}`,
        },
      ],
      output_config: {
        format: {
